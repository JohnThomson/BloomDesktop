...
describe("audio recording tests", () => {
    ...
    fit("makeAudioSentenceElementsLeaf preserves color markup", () => {
        const recording = new AudioRecording();
        ...
        for (let i = 0; i < 3; i++) {
            expect(colorSpans[i].parentElement).toBe(sentences[i]);
        }
        expect(colorSpans[0].innerText).toBe("One.");
        expect(colorSpans[1].innerText).toBe("Two.");
        expect(colorSpans[2].innerText).toBe("Three");
    });
