...
namespace Bloom.ToPalaso
{
    ...
    public static class XmlExtensions
    {
        ...
        public static XmlElement ParentOrSelfWithClass(this XmlElement start, string targetClass)
        {
            ...
            while (
                current != null
                && !(" " + (current.Attributes["class"]?.Value ?? "") + " ").Contains(
                    " " + targetClass + " "
                )
            )
        ...
        /// </summary>
        /// <returns></returns>
        public static XmlElement AncestorWithAttributeValue(
            this XmlElement start,
            string targetAttr,
            string targetVal
        )
        {
        ...
        public static XmlElement GetChildWithName(this XmlElement xmlElement, string name)
        {
            return xmlElement.ChildNodes
                    .Cast<XmlNode>()
                    .FirstOrDefault(n => n.Name.ToLowerInvariant() == name) as XmlElement;
        }
