...
namespace Bloom.Api
{
    ...
    public class BloomServer : IBloomServer, IDisposable
    {
        ...
        private string ProcessPath(string localPath, string modPath)
        {
            ...
            if (
                RobustFile.Exists(possibleFullImagePath) && Path.IsPathRooted(possibleFullImagePath)
            )
            {
        ...
        private bool ProcessCssFile(IRequestInfo info, string incomingPath)
        {
            ...
            if (RobustFile.Exists(localPath) && Path.IsPathRooted(localPath))
            {
                ...
                if (
                    (cssFile == "defaultLangStyles.css")
                    || (cssFile == "customCollectionStyles.css")
                    ||
                    // Temporary for 5.6 only: if not legacy, use the basePage.css that came with the book.
                    (
                        cssFile == "basePage.css"
                        && (
                            _bookSelection?.CurrentSelection?.Storage.ShouldUseCurrentBasePageCss()
                            ?? false
                        )
                    )
                )
