...
namespace Bloom.Book
{
    ...
    public interface IBookStorage
    {
        ...
        // Temporary for 5.6 only
        bool ShouldUseCurrentBasePageCss();
    }
    ...
    public class BookStorage : IBookStorage
    {
        ...
        public void UpdateSupportFiles()
        {
            ...
            else
            {
                ...
                {
                    // Temporary for 5.6 only
                    if (supportFile == "basePage.css" && ShouldUseCurrentBasePageCss())
                        continue;
        ...
        public void Update(string fileName, string factoryPath = "")
        {
            ...
            {
                if (
                    fileName.ToLowerInvariant().Contains("xmatter")
                    && !fileName.ToLower().StartsWith("factory-xmatter")
                )
                {
        ...
        // Temporary for 5.6 only
        public bool ShouldUseCurrentBasePageCss()
        {
            var appearanceJsonPath = Path.Combine(FolderPath, "appearance.json");
            if (!File.Exists(appearanceJsonPath))
                return false;
            var appearanceJson = File.ReadAllText(appearanceJsonPath);
            var appearance = DynamicJson.Parse(appearanceJson);
            var theme = appearance.cssThemeName;
            if (theme.StartsWith("legacy"))
                return false;
            var appearanceCssPath = Path.Combine(FolderPath, "appearance.css");
            if (!File.Exists(appearanceCssPath))
                return false;
            var basePageCssPath = Path.Combine(FolderPath, "basePage.css");
            if (!File.Exists(basePageCssPath))
                return false;
            return true;
        }
    }
